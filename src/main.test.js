...
describe('reverseObject', () => {
  ...
  it('should reverse multiple properties', () => {
    ...
    expect(reverseObject({ a: 1, b: 2, c: 3 })).to.eql({
      1: 'a',
      2: 'b',
      3: 'c',
    });
...
describe('isNumber', () => {
  ...
  it('should return false for everything', () => {
    ...
    expect(isNumber(true)).to.eql(false);
  });
});
...
describe('isArray', () => {
  it('should return true for array', () => {
    ...
    expect(isArray([1])).to.eql(true);
  });

  it('should return false for everything', () => {
    expect(isArray({})).to.eql(false);
    expect(isArray('a')).to.eql(false);
    expect(isArray(1)).to.eql(false);
    expect(isArray(true)).to.eql(false);
  });
});
...
describe('isObject', () => {
  it('should return true for object', () => {
    ...
    expect(isObject({ a: 1 })).to.eql(true);
    expect(isObject([])).to.eql(true);
  });
...
describe('map', () => {
  ...
  it('should not mutate the input array', () => {
    const numbers = [1, 2, 3];
    const duplicatedThroughMap = map(numbers, (el) => el);
...
describe('filter', () => {
  ...
  it('should filter all odd values in object', () => {
    const evens = filter(
      { a: 1, b: 2, c: 3, d: 4 },
      (value, key, collection) => value % 2 !== 0,
    );
  ...
  it('should not mutate the input array', () => {
    ...
    const duplicatedThroughFilter = filter(numbers, () => true);
...
describe('reject', () => {
  ...
  it('should not mutate the input array', () => {
    const numbers = [1, 2, 3, 4];
    const duplicatedThroughReject = filter(numbers, () => false);
...
describe('reduce', () => {
  ...
  it('should not mutate the input array', () => {
    const numbers = [1, 2, 3];
    const hi = reduce(numbers, (a, b) => a + b);
    expect(numbers).to.eql([1, 2, 3]);
  });
});
...
describe('extend', () => {
  ...
  it('in the case of a conflict, it should use the last property\'s values', () => {
    expect(extend({ x: 'x' }, { a: 'a', x: 2 }, { a: 1 })).to.eql({
      x: 2,
      a: 1,
    });
  });
  ...
  it('should copy undefined values', () => {
    ...
    expect('a' in extended && 'b' in extended).to.eql(true);
  });
});

describe('isString', () => {
  it('should return true for strings', () => {
    expect(isString('a')).to.eql(true);
    expect(isString('5')).to.eql(true);
  });

  it('should return false for everything', () => {
    expect(isString({})).to.eql(false);
    expect(isString([])).to.eql(false);
    expect(isString(5)).to.eql(false);
...
describe('cloneDeep', () => {
  ...
  it('should be able to clone an unknown amount of layers deep', () => {
    ...
    expect(deepClone[2].friends.friends.user).to.equal(
      users[2].friends.friends.user,
    );
    expect(
      deepClone[2].friends.friends.friends.friends.friends.friends.friends,
    ).to.eql(users[2].friends.friends.friends.friends.friends.friends.friends);
    expect(
      deepClone[2].friends.friends.friends.friends.friends.friends.friends,
    ).to.not.equal(
      users[2].friends.friends.friends.friends.friends.friends.friends,
    );
    expect(
      deepClone[2].friends.friends.friends.friends.friends.friends.friends.user,
    ).to.equal(
      users[2].friends.friends.friends.friends.friends.friends.friends.user,
    );
  });
});
...
describe('memoize', () => {
  ...
  it('should work with objects as arguments', () => {
    ...
    expect(fastTimeCheck({ foo: 'bar' })).to.not.equal(
      fastTimeCheck({ different: 'result' }),
    );
  });
  ...
  it('should work with arrays as arguments', () => {
    ...
    expect(fastTimeCheck(['foo', 'bar'])).to.equal(secondTime);
    expect(fastTimeCheck(['foo', 'bar'])).to.not.equal(
      fastTimeCheck(['different', 'results']),
    );
  });
});
...
describe('throttle', () => {
  ...
  it('throttled functions called repeatedly should adhere to time limitations', (done) => {
    ...
    setTimeout(() => {
      ...
      expect(results[2]).to.eql(undefined);
      expect(results[3]).to.eql(2);
      expect(results[4]).to.eql(undefined);
...
describe('partition', () => {
  it('should split the array into two based on predicate', () => {
    expect(partition([0, 1, 2, 3, 4, 5], (num) => num % 2 === 0)).to.eql([
      [0, 2, 4],
      [1, 3, 5],
    ]);
    expect(partition([0, 1, 2, 3, 4, 5], (num) => num % 2 === 1)).to.eql([
      [1, 3, 5],
      [0, 2, 4],
    ]);
  });
  ...
  it('should not mutate the input array', () => {
    ...
    const partitioned = partition(numbers, (num) => num % 10 === 0);
